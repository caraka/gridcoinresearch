...
ls "$SRCDIR"/*.dsc >/dev/null 2>&1 || { echo "::error::no .dsc in $SRCDIR"; exit 1; }

VER=$(basename "$(ls "$SRCDIR"/*.dsc | head -1)" | sed -E 's/.*_([^_]+)\.dsc$/\1/')
...
( cd "$WC"
  ...
  osc addremove
  osc commit -m "CI: ${PACKAGE} ${VER} (${GITHUB_REF_NAME:-manual}@${GITHUB_SHA:0:8})"
)
